...
The functional mechanism of the node relies on an asynchronous callback pipeline. The node subscribes to the raw image telemetry broadcast by the spatial camera on the \texttt{oakd/rgb/preview\newline/image\_raw} topic. Upon receiving a standardized ROS \texttt{sensor\_msgs/msg/Image} message, the node utilizes the \texttt{cv\_bridge} library to convert the image matrix into a BGR format compatible with OpenCV.
...
\begin{figure}[!ht]
\centering
\includegraphics[width=0.7\linewidth]{figures/object_locator.png}
...
\item \textbf{Camera Info Sub}: Subscribes to the \texttt{/oakd/rgb/preview/camera\_info} topic using the \newline\texttt{sensor\_msgs/msg/CameraInfo} message type to extract lens intrinsic parameters required for spatial back-projection.
\item \textbf{Stereo Depth Sub}: Subscribes to the \texttt{/oakd/stereo/image\_raw} topic using the \newline \texttt{sensor\_msgs/msg/Image} message type to acquire the structured depth matrices calculated by the camera.
\item \textbf{RGB Image Sub}: Subscribes to the \texttt{/oakd/rgb/preview/image\_raw} topic, also utilizing the \texttt{sensor\_msgs/msg/Image} type, to capture the live color frames for alignment and visualization.
\end{itemize}
...
Finally, to streamline real-time validation and debugging during experimental testing, a dedicated ROS~2 parameter named \texttt{enable\_viz} was integrated into the node configuration, defaulting to a boolean value of \texttt{False}. When a user explicitly toggles this configuration flag to \texttt{True}, an optional visualization sub-module is triggered. This module spins up an additional publisher targeting the \texttt{/yolo/debug\_image} topic with a \texttt{sensor\_msgs/msg/Image} payload. This debug stream overlays the calculated spatial coordinates, distance metrics, and bounding-box targets directly onto the live color frame for visual verification. %An example layout for this visual validation is reserved in Figure~\ref{fig:locator_debug}.
...
While the theoretical behavior and conditions of the five operational states (\texttt{INIT}, \texttt{SEARCHING}, \newline \texttt{FOLLOWING}, \texttt{CLOSE}, and \texttt{LOST}) are established in Section 4.6, their software-technical implementation is realized through a native Python class using a \texttt{match-case} control structure. The asynchronous callback routines handle incoming vision coordinates and actively manage the lifecycle of the \texttt{Nav2} action clients. To ensure that transient line-of-sight obstructions do not destabilize the network, the \texttt{State.LOST} routine utilizes a non-blocking hardware timer thread. The core code layout, object initializations, and the coordinate transformation functions of this node are referenced in Listing~\ref{lst:fsm_core}.

\newpage

\lstinputlisting[linerange={308-323},language=Python,caption={Initialization and PointStamped-to-PoseStamped conversion logic inside the person\_follower node.},label={lst:fsm_core}]{codes/person_follow.py}

Additionally, the state transition logic and conditional execution loops governing the operational state jumps are mapped in Listing~\ref{lst:fsm_transitions_code}.

\lstinputlisting[linerange={100-262},language=Python,caption={Match-case control structure handling the asynchronous state transitions and Nav2 action dispatches.},label={lst:fsm_transitions_code}]{codes/person_follow.py}
